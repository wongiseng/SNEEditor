...
YAHOO.lang.extend(WireIt.BezierWire, WireIt.Wire, {
   ...
   draw: function() {
      ...
      // Get the positions of the terminals
	  if(!this.terminal1 || !this.terminal2) return;
      var p1 = this.terminal1.getXY();
      var p2 = this.terminal2.getXY();
...
inputEx.Field = function(options) {
	 ...
	 */
	this.updatedEvt = new util.CustomEvent('updated', this);8911
...
WireIt.BaseEditor.prototype = {
	 ...
	 */
  	render: function() {
	    ...
	    // **AWSMOD** just a marker of my modification.
	    // Should have been done by extending this class instead of messing around here.
	    this.tabViews = new YAHOO.widget.TabView("center");
	    
	    this.graphTab = new YAHOO.widget.Tab( {
			active : true,
			label : 'Graph View',
			contentEl : Dom.get("graph-content")
	    });
	    
	    this.owlTab = new YAHOO.widget.Tab( {
			label : 'OWL View',
			contentEl : Dom.get("owl-content")
	    });

	    /* for debugging only
	    this.modTab = new YAHOO.widget.Tab( {
			label : 'Modules View',
			contentEl : Dom.get("mod-content")
	    });
	    
	    this.wireTab = new YAHOO.widget.Tab( {
			label : 'Wires View',
			contentEl : Dom.get("wire-content")
	    }); */
	    
	    
	    
	    this.tabViews.addTab(this.graphTab);
	    this.tabViews.addTab(this.owlTab);

	    /* For debugging only
	    	this.tabViews.addTab(this.modTab);
	    	this.tabViews.addTab(this.wireTab);
	    */
	    
	    
		this.owlTab.addListener("click", this.updateOWLView);
		
		/* Debugging only
			this.modTab.addListener("click", this.updateModView);
			this.wireTab.addListener("click", this.updateWireView);
			this.modTab.editor = this;
			this.wireTab.editor = this;

		 */		
		
		this.owlTab.editor = this;
  ...
  updateWireView: function(){ },
	...
	renderPropertiesAccordion: function() {
		...
		// Open all panels
		for(var l = 1, n = this.accordionView.getPanels().length; l < n ; l++) {
			this.accordionView.openPanel(l);
		}
...
WireIt.WiringEditor = function(options) {
	...
	WireIt.WiringEditor.superclass.constructor.call(this, options);
	 ...
	 //AWS maybe this is the one which loads at first.
	 //this.load();
};
...
lang.extend(WireIt.WiringEditor, WireIt.BaseEditor, {
	 ...
	 setOptions: function(options) {
	    ...
	    this.options.layerOptions.parentEl = layerOptions.parentEl ? layerOptions.parentEl : Dom.get("graph-content");
	 ...
	 */
  	render: function() {
	     ...
	     */
		 ...
		 YAHOO.util.Dom.setStyle(this.layer.parentEl, "height", "600px");
	...
	renderModulesAccordion: function() {
 		//AWS: I'll render this myself somewhere else.
		...
		if(!Dom.get('modulesAccordionView')) {
			...
			for(i=0;i<SNE.repositories.length;i++){
				var li = WireIt.cn('li');
				li.appendChild(WireIt.cn('h2',null,null, SNE.repositories[i].label));
				var d = WireIt.cn('div');
				d.appendChild( WireIt.cn('div', {id: SNE.repositories[i].el}) );
				li.appendChild(d);
	...
	addModule: function(module, pos) {
	    try {
	       ...
	       //AWS: Trying to send id information up to the backend.
	       containerConfig.id = module.id;
 	...
 	save: function() {
    	...
    	if(!value || value.name === ""  ) {
    		this.alert("Please give name to this topology" );
	...
	inputFilterTimer: function() {
	 ...
	 /**
	  * Pipe here actually means Network Topology on our case
	  * @method loadPipe
	  * @param {String} name Pipe name
	  */
	 loadPipe: function(name) {
		 // Here *this* means SNE.editor
	    ...
	    this.loadThisWiring(wiring);
	 },
	 
	 /* AWS: I split this part because I am going to use it in upload*/
	 loadThisWiring: function(wiring){
		 try {
				if(!wiring) {
			    ...
			    if(lang.isArray(wiring.modules)) {
			       ...
			       for(i = 0 ; i < wiring.modules.length ; i++) {
			          ...
			          if(this.modulesByID[m.id]) {  
			             //var baseContainerConfig = this.modulesByName[m.name].container;
			        	 var baseContainerConfig = this.modulesByID[m.id].container;
			        	 
			             YAHOO.lang.augmentObject(m.config, baseContainerConfig);
			             
			             m.config.title = m.name;
			             // AWS: I guess this is important to keep the ID on loaded modules
			             m.config.id = m.id; 
			             ...
			             throw new Error("WiringEditor: module '"+m.id+"' not found !");
			          }
			       }
				...
				// This is setting a value to this (editor) which will be used in SNEEditor.js 
				// when generating OWL
				
				if(typeof(this.getValue) === 'undefined') {
						var newValue = {}
						newValue.working = wiring;
						newValue.name = wiring.properties.name;
						
						this.getValue = function(){return newValue;};
				}
				
		  	} // end try
		  	catch(ex) {
		  		//AWS: This is temporary, I will fix it later.
		     	//this.alert(ex);
				if(window.console && YAHOO.lang.isFunction(console.log)) {
					//console.log(ex);
				}
 ...
 getValue: function() {
   ...
   for( i = 0 ; i < this.layer.containers.length ; i++) {
	  //AWS: Adding ID into this so that it can be used later on
      obj.modules.push( {name: this.layer.containers[i].title, id : this.layer.containers[i].id, value: this.layer.containers[i].getValue(), config: this.layer.containers[i].getConfig()});
   }
