...
/**
 * This class is responsible for generating WireIT classes from an OWL/RDF files.
 * It uses OWL/API to parse the RDF/OWL files.
 * 
 * It also will look for a special generated comment section where loaction of modules is stored.
 * @author wibisono
 *
 */
public class WireItGraphConstructor
{
	...
	/**
	 * This constructor will generate a WireIT representation based on OWL/RDF file defined in fileData
	 * @param filedata
	...
	public WireItGraphConstructor(String filedata, String filename) throws OWLOntologyCreationException
	{
		this(filedata);
		fileName = filename;
	...
	{			
		...
		for(OWLClass owlClass : classes){
			...
			for(OWLIndividual individu : individuals){
				...
				Set <OWLAnnotationAssertionAxiom> axioms = individu.asOWLNamedIndividual().getAnnotationAssertionAxioms(theOntology);
				
				
				String positionAnnotationStr = "";
				for(OWLAnnotationAssertionAxiom posAx : axioms){
					OWLAnnotation posAnot = posAx.getAnnotation();
					if(posAnot.getValue() instanceof OWLLiteral){
						OWLLiteral val = (OWLLiteral) posAnot.getValue();
						String literal =val.getLiteral(); 
						if(literal.contains("Position : "))
							positionAnnotationStr = literal.substring(literal.lastIndexOf(":")+1);
					}
				}
				...
				appendLine(1, result, "{");
    				...
    				if(positionAnnotationStr.length() == 0){
    					// No Position Annotation String, we generate position 
    	            	appendLine(3, result, quote("position")+" : ["+posX+","+ posY+"],");
    				} else
    					appendLine(3, result, quote("position")+" : "+positionAnnotationStr + ",");
		...
		appendLine(result, "}");
		System.out.println("When generating wires :"+result);
	...
	public static void main(String[] args) throws IOException, OWLOntologyCreationException
	{
		...
		BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(new File("downloadwire-less.owl"))));
