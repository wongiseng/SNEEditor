...
public class SesameConnector
{
	...
	{
		return doQueryString("PREFIX rdfs:<http://www.w3.org/2000/01/rdf-schema#>"
								+ "PREFIX owl:<http://www.w3.org/2002/07/owl#>"
								+ "PREFIX rdf:<http://www.w3.org/1999/02/22-rdf-syntax-ns#>"
								+ "SELECT DISTINCT ?ObjectProperty ?Domain ?Range ?Comment"
	...
	public String doQueryString(String queryString){
		MultivaluedMap<String, String> queryParams = new MultivaluedMapImpl();
		queryParams.add("queryLn", "SPARQL");
		queryParams.add("query", queryString);
	...
	public static String addN3Statements(String statements, String endPoint)
	{
		StringBuffer result = new StringBuffer();
		URL url;
		URLConnection urlConn;
		...
		try {
			...
			printout.writeBytes(statements);
			printout.flush();
			printout.close();
